...
function AuthPage() {
  ...
  // Discord isn't in the Lovable-hosted provider list (google/apple/microsoft/
  // lovable only), so it goes through Supabase's own OAuth flow directly —
  // this requires the Discord provider to be turned on in the Supabase
  // dashboard (Authentication → Providers → Discord) with a Client ID/Secret
  // from the Discord Developer Portal. Scopes mirror the original app:
  // identify + email so the profile trigger can read a name/avatar, plus
  // guilds.join to allow auto-joining the Discord server later if wired up.
  async function discord() {
    ...
    if (error) {
      toast.error("เข้าสู่ระบบด้วย Discord ไม่สำเร็จ");
    }
    // On success the browser gets redirected away to Discord, so there's
    ...
    <div className="mx-auto flex max-w-md flex-col px-4 py-16">
      <div className="surface-card rounded-3xl p-8">
        ...
        <div className="flex flex-col gap-2">
          <Button variant="secondary" className="w-full rounded-full" onClick={discord}>
