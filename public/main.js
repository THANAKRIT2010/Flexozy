...
(function(){
  ...
  window.showAdminPinModal = function(){
    ...
    overlay.querySelector('#adminPinCancel').addEventListener('click', ()=>{ close(); showView('scripts'); location.hash = '#/scripts'; });
...
function route(){
  const raw = location.hash.replace(/^#\/?/,'') || 'scripts';
  const rawVaultMatch = raw.match(/^raw\/vault\/([^/?]+)/);
  if (rawVaultMatch) {
    // ลิงก์รูปแบบเก่า (#raw/vault/CODE) — พาไปที่โดเมน api.flexozy.xyz แทน เพื่อให้ใช้กับ loadstring ได้จริง
    location.replace(vaultRawUrl(rawVaultMatch[1]));
    return;
  }
  ...
  if (view === 'admin' && param) { currentAdminSubtab = param; if (document.getElementById('adminPanel')) adminTab(param); }
}
function showView(view){
  const known = ['scripts','store','vault','roblox','team','partners','admin'];
  ...
  if (view === 'admin') {
    ...
    if (!panel) {
      // หน้านี้ไม่มีแผงแอดมินอยู่ในตัว (คือ index.html ไม่ใช่ /admin.html) ส่งไปหน้าแอดมินจริงแทน
      // ป้องกันด้วยเซิร์ฟเวอร์อยู่แล้วว่าใครเข้า /admin.html ได้บ้าง หน้านี้แค่ redirect ให้ถูกที่
...
function loginWithDiscord(){ window.location.href = '/login.html'; }
...
async function submitVault(e){
  ...
  try{
    ...
    if (res.ok){
      ...
      document.getElementById('vaultResultLink').value = link;
      document.getElementById('vaultResult').classList.remove('hidden');
      const hintEl = document.getElementById('vaultResultHint');
      hintEl.textContent = data.has_password
        ? `ใช้กับ loadstring ได้เลยโดยไม่ต้องใส่รหัส: loadstring(game:HttpGet("${link}"))()  — รหัสผ่านจะใช้ป้องกันแค่การดู/คัดลอกโค้ดผ่านหน้าเว็บเท่านั้น`
        : `ใช้กับ loadstring ได้เลย: loadstring(game:HttpGet("${link}"))()`;
...
function goAdminTab(subtab){
  ...
  if (document.getElementById('adminPanel')) {
    // อยู่ในหน้า /admin.html อยู่แล้ว แค่สลับแท็บในหน้าเดียวกัน
    location.hash = '#/admin/' + subtab;
    showView('admin');
    applyAdminTabVisibility();
    adminTab(subtab);
  } else {
    // อยู่หน้าเว็บหลัก (index.html) — หน้านี้ไม่มีแผงแอดมินแล้ว ต้องไปหน้า /admin.html จริงๆ
    location.href = '/admin.html#/admin/' + subtab;
  }
}
